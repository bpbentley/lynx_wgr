...
data<-list.files("popgen/pcangsd/logs", pattern = "likelihoods", full.names = T)
klist<-list()
for(q in 1:length(data)){
  df<-read.table(file=data[q], sep = " ")
  K=as.numeric(gsub("K","",gsub("_.*","",df[,1])))
  like=as.numeric(df[,2])
  rep=gsub(".*_","",gsub("\\..*","",df[,1]))
...
for(k in 2:6){
  ...
  K2 <- read_delim(file = paste0("popgen/pcangsd/Q/lynx_admix_K",k,"_",best_run,".admix.",k,".Q"),delim = " ", col_names = F)
  ...
  ggsave(paste0("plots/PCAngsd/PCAngsd_K",k,".svg"), p1, height = 4, width = 10)
  ggsave(paste0("plots/PCAngsd/PCAngsd_K",k,".png"), p1, height = 5, width = 10)
}
